    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-slate-100 to-white print:bg-white">
      ...
      <div className="hidden print:block w-[80mm] mx-auto px-4 pt-2 pb-4 bg-white text-black font-bold"
        ...
        <div className="text-center mb-2" style={{ borderBottom: '1.5px dashed #aaa', paddingBottom: 6 }}>
          <h2 className="text-[20px] font-black tracking-wider mt-1">HÓA ĐƠN THANH TOÁN</h2>
          ...
          {invoice.customerName && (
            <p className="text-[11px] tracking-wide">Khách: {invoice.customerName}</p>
          )}
          <p className="text-[11px] text-gray-600">{new Date(invoice.createdAt).toLocaleString('vi-VN')}</p>
        </div>
        {/* Bảng chi tiết */}
        <table className="w-full text-[12px] mb-2" style={{ borderCollapse: 'collapse', lineHeight: 1.6 }}>
          ...
          <tbody>
            ...
            {/* Các món */}
            {(invoice.items || []).map((item: any, index: number) => (
              <tr key={index} style={{ borderTop: '1px dashed #ddd' }}>
