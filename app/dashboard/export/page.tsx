...
export default function ExportPage() {
    ...
    const handleQuantityBlur = (id: string) => {
        ...
        if (!isNaN(qty)) {
            handleUpdateCartItem(id, { quantity: qty });
        }
        setEditingQuantities((prev) => { const n = { ...prev }; delete n[id]; return n; });
    };
                                    ...
                                    <div key={item.productId} className="flex items-center justify-between gap-3 bg-slate-50 p-3 rounded-2xl">
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex items-center gap-0.5">
                                                <input
                                                    type="text"
                                                    className="w-20 text-xs font-medium text-slate-400 bg-transparent border-none outline-none focus:bg-white rounded px-1 transition-colors"
                                        ...
                                        <div className="flex items-center gap-1.5">
                                            <div className="flex items-center bg-white rounded-lg p-1">
                                                <button onClick={() => updateQty(item.productId, -1)} className="w-7 h-7 flex items-center justify-center text-slate-400 active:scale-90 transition-transform"><Minus className="w-3 h-3" /></button>
                                                <input
                                                    type="text"
                                                    className="w-8 text-center text-sm font-black text-slate-700 bg-transparent outline-none"
                                                    value={editingQuantities[item.productId] ?? item.quantity}
                                                    onChange={(e) => handleQuantityChange(item.productId, e.target.value)}
                                                    onBlur={() => handleQuantityBlur(item.productId)}
                                                />
                                                <button onClick={() => updateQty(item.productId, 1)} className="w-7 h-7 flex items-center justify-center text-indigo-600 active:scale-90 transition-transform"><Plus className="w-3 h-3" /></button>
                                            </div>
                                            <button
